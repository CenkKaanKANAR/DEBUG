...
void CCU_Table::receiveUdpInMessage(QByteArray recv_data)
{
    ...
    QByteArray firstSixBytes = recv_data.mid(0, 6);

    qDebug() << "Port Id: " << QString::number(port_id, 16);  // Port Id'yi hexadecimal olarak görmek için
    qDebug() << "CONFIG ID BYTTE DEGERİİİİİ: " << firstSixBytes;

    if(port_id == SKA1_DD_RIOM_INPUTS)
    {
        qDebug("ALDIĞIM DATA SKA1_DD_RIOM VE PORT ID Valid Port Id: %x",port_id);
        //m_ConfigWidget->updateEmulatorConfigStruct();
        m_ConfigWidget->setMessageConfigStruct(firstSixBytes);
    ...
    CcuTableHandler::handleIncomingCcuInSystemUdpPacketes(ccu_in, recv_data,m_ConfigWidget);
}
